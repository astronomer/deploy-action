...
1. Create a GitHub Actions workflow in your repository that uses the latest version of this action. For example, `astronomer/deploy-action@v0.14.0`.
...
| `cli-version` | `` | The desired Astro CLI version to use. If left unset, the action installs the default version of its pinned installer script, currently 1.44.0. This applies only when the Astro CLI is not installed already. |
...
4. `image-only`: In this mode, deploy-action would run through all the file changes:
  - if there are no file changes in the configured root-folder then it skips deploy
  - otherwise it would deploy only the image, leaving existing DAGs on the deployment untouched
