...
# hack to mock git commands as part of action.yaml so that we could simulate a
# path that shares the dag-folder prefix without actually being inside it

# Check if the script was invoked with "git diff"
if [[ "$1" == "diff" ]]; then
  echo "dags-old/exampledag.py"
elif [[ "$1" == "fetch" ]]; then
